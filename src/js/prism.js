...
(function () {
	...
	Prism.plugins.toolbar.registerButton('download-sample', function (env) {
		let pre = env.element.parentNode;
		if (!pre || !/pre/i.test(pre.nodeName) || !pre.hasAttribute('data-src') || !pre.hasAttribute('data-download-link')) {
			return;
		}
		let src = pre.getAttribute('data-src') || false;
		let src_name = "";
		if(src){
			let content = env.element.textContent;
			let blob = new Blob([content], {
				type: 'text/plain'
			});
			src_name = src.split("/")[src.split("/").length-1];
			let a = document.createElement('a');
			a.href = URL.createObjectURL(blob);
			a.setAttribute('download', src_name);
